# Rajarshi Giri

I am a 4th-year BS-MS undergraduate majoring in Physics at <a href="https://www.iiserkol.ac.in/">**IISER Kolkata**</a>. <br>
I am broadly interested in many-body physics, nonequilibrium quantum dynamics, and statistical mechanics.

Currently, I am working in a project on **quantum chaos** in  a floquet driven many-body model in *<a href="https://sites.google.com/cornell.edu/sayan">Theory of Quantum and Ultracold Matter</a>* group of Prof. Sayan Choudhury at <a href="https://www.hri.res.in">**HRI Allahabad (Prayagraj)**</a>. <br>  In the past, I have worked with <a href="https://www.imsc.res.in/~padmanath/index.html#/">Dr. Padmanath Madanagopalan</a><br> in <a href="https://www.imsc.res.in/">**IMSc, Chennai**</a> <br> on a method of reducing variance in large time two-point correlations obtained from MC simulations, in scalar field theory, and with <a href="https://sites.google.com/iiti.ac.in/priyanka-singh">Prof. Priyanka Singh</a>, <a href="https://www.iiti.ac.in/">**IIT Indore**</a> remotely on modelling of gas properties in dark matter halos.
<p>
  <a href="mailto:rg23ms099@iiserkol.ac.in">Email</a> &nbsp;/&nbsp;
  <a href="https://phy-lic.github.io">Website</a> &nbsp;/&nbsp;
  <a href="https://linkedin.com/in/rajarshi-giri">LinkedIn</a>
</p>

---

### Selected Projects

<table border="0" style="width: 100%; border-collapse: collapse;">
<tr>
<td width="35%" valign="top" style="border: none; padding-right: 20px; padding-bottom: 30px;">
<img src="https://github.com/user-attachments/assets/69269a2c-3f1d-4efc-ba61-790f4c89810f" alt="husimi" width="100%" style="border-radius: 4px; border: 1px solid #d0d7de;">
</td>

<!-- <img width="504" height="435" alt="image" src="https://github.com/user-attachments/assets/69269a2c-3f1d-4efc-ba61-790f4c89810f" /> -->

<!-- <img width="539" height="480" alt="image" src="https://github.com/user-attachments/assets/216eebc1-e5b6-4e69-a38a-84bc58bbdad8" /> -->

  <!-- <img width="255" height="226" alt="image" src="https://github.com/user-attachments/assets/a407181a-253f-4c95-844f-da0c5a32d454" /> -->

<td width="65%" valign="top" style="border: none; padding-bottom: 30px;">
<!-- Currently working on a project on quantum chaos on driven many-body systems.  -->
<strong>Quantum chaos in a kicked Bose-Hubbard dimer model</strong> <br>
  I studied the integrability to chaos crossover in a kicked BH dimer model (which has a semicalassical counterpart) using spectral statistics and multifractality dimension analysis and studied rich local phase space dynamics using quantum information tools like OTOCs, entanglement entropy.<br> <a href=".">presentation</a> <br>
  </td>
</tr>

<tr>
<td width="35%" valign="top" style="border: none; padding-right: 20px; padding-bottom: 30px;">
<img src="https://github.com/user-attachments/assets/e4e9b40c-b23a-42f6-8eb6-9b4a7afdd68e" alt="bosons3" width="100%" style="border-radius: 4px; border: 1px solid #d0d7de;">
</td>
<td width="65%" valign="top" style="border: none; padding-bottom: 30px;">
  
**Effect of Disorder and Interactions in two site BH model** <br>
I studied the (counter-intuitive) phenomena of interaction-induced coherence in a disordered two-site Bose-Hubbard model. This minimal model captures enhancement of the superfluid-like regimes seen in full Bose-Hubbard model.<br> <a href="https://phy-lic.github.io/disordered-bose-hubbard/disordered_BH_presentation_for_gh.pdf">presentation</a> <br> 
</td>
</tr>
<tr>
<td width="35%" valign="top" style="border: none; padding-right: 20px; padding-bottom: 30px;">
<img src="https://github.com/user-attachments/assets/ffe23d3d-b528-43cc-9128-3864dbc0175a" alt="imsc_plot_gh" width="100%" style="border-radius: 4px; border: 1px solid #d0d7de;">
</td>
<td width="65%" valign="top" style="border: none; padding-bottom: 30px;">

**Variance Reduction in Large Time Correlations in Scalar Field Theory**<br>
  We studied the signal-to-noise problem in large-time correlation functions of lattice scalar field theory. Using the analytically known distribution of bilocal correlators, we implemented a variance-reduction method that improves the extraction of effective masses from Monte Carlo simulations.<br> <a href="https://phy-lic.github.io/phi4-lattice-simulation/IMSc_report_for_gh.pdf">report</a>
<br>

</td>
</tr>
<tr>
<td width="35%" valign="top" style="border: none; padding-right: 20px; padding-bottom: 10px;">
<img src="https://github.com/user-attachments/assets/cf4f12d0-a3cc-4009-8d6d-ccc9098da071" alt="psingh_plot2_gh" width="100%" style="border-radius: 4px; border: 1px solid #d0d7de;">
</td>
<td width="65%" valign="top" style="border: none; padding-bottom: 10px;">
<!-- Worked on review project under the supervision of Dr. Singh. <br> -->
I studied the Komatsu–Seljak model of intracluster gas in dark matter halos. Derived universal gas density and temperature profiles from hydrostatic equilibrium assumption and a polytropic equation of state.<br> <a href="https://github.com/phy-lic/dark-matter-gas-profile/psingh_work_draft.pdf">report</a><br>
Studied the Press–Schechter model of hierarchical structure formation and the resulting dark matter halo mass function. Investigated its dependence on cosmological parameters and cosmic time. <br><a href="https://github.com/phy-lic/dark-matter-gas-profile/psingh_psmf.pdf">notes</a><br>
</td>
</tr>
</table>

---

### Course Projects

<table border="0" style="width: 100%; border-collapse: collapse;">
<tr>
<td width="35%" valign="top" style="border: none; padding-right: 20px;">
<img src="https://github.com/user-attachments/assets/2531544d-8f0e-4c6e-8cd3-0f251dcd5bf0" alt="sourin_term_paper_plot1_gh" width="100%" style="border-radius: 8px; border: 1px solid #d0d7de;">
</td>
<td width="65%" valign="top" style="border: none;">

**Entanglement as a Resource** <br>
In this project, we review the conceptual foundations of quantum entanglement and its role as a nonclassical resource in communication and cryptography.
We then demonstrate its practical applications through Superdense Coding, Quantum Teleportation and Quantum Key Distribution (BB84 and E91) using Qiskit and IBM Quantum hardware.
<br>
<br> <a href="https://github.com/phy-lic/entanglement-as-resource/sourin_term_paper_presentation.pdf">presentation</a> / <a href="https://github.com/phy-lic/entanglement-as-resource/sourin_term_paper_for_gh.pdf">report</a>

</td>
</tr>
</table>

---

### My Workhorses

![Julia](https://img.shields.io/badge/Julia-9558B2?style=for-the-badge&logo=julia&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white)
![LaTeX](https://img.shields.io/badge/latex-%23008080.svg?style=for-the-badge&logo=latex&logoColor=white)
![Typst](https://img.shields.io/badge/typst-%23239DAD.svg?style=for-the-badge&logo=typst&logoColor=white)
![Fedora](https://img.shields.io/badge/Fedora-%23294172.svg?style=for-the-badge&logo=fedora&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-%23181717.svg?style=for-the-badge&logo=github&logoColor=white)

<br><br>
